#ifndef CPP_QT_TPMINIPROJET_COLOH_H
#define CPP_QT_TPMINIPROJET_COLOH_H
...
namespace Color {
    ...
    public:
        ...
        Modifier& operator<<(ForegroundColor color) {
            codes.append(static_cast<Code>(color));
            return *this;
        }
        ...
        friend QDebug operator<<(QDebug debug, const Modifier& mod) {
            debug.nospace() << "\033[";
            const int size = mod.codes.size();
            for (int i = 0; i < size; ++i) {
                debug << mod.codes.at(i);
                if (i < size - 1)
                    debug << ';';
            }
            debug << 'm';
...
#endif // CPP_QT_TPMINIPROJET_COLOH_H
